...
void process_input(const std::string &filename)
{
    ...
    max_overall_strength =  max_length = max_strength_among_longest = 0U;
    recur(0, 0, 0);
}
...
int main()
{
    ...
    assert(max_overall_strength == 31);
    assert(max_strength_among_longest == 19);

    process_input("input.txt");
    std::cout << "Part 1: " << max_overall_strength << std::endl;
    std::cout << "Part 2: " << max_strength_among_longest << std::endl;

    assert(max_overall_strength == 1859);
    assert(max_strength_among_longest == 1799);
    return 0;
}
