...
const auto grid_size = 300U;

int grid[grid_size + 1][grid_size + 1];

auto compute_power(unsigned x, unsigned y, int gsn)
{
    auto rack_id = x + 10;
...
auto get_power_of_square(unsigned x, unsigned y, unsigned n)
{
    auto power = 0;
    for (auto i = 0U; i < n; i++)
        for (auto j = 0U; j < n; j++)
...
auto find_square_with_highest_power(unsigned min_n, unsigned max_n)
{
